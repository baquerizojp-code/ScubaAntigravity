...
export default function CenterDetail({ id }: { id: string }) {
  ...
  > = {
    ...
    delete: {
      ...
      desc: t('superAdmin.confirmDeleteDesc'),
      destructive: true,
    },
  };
    ...
    <div className="max-w-2xl">
      ...
      <div className="flex items-start justify-between gap-4 mb-6">
        <div>
          ...
          <Badge
            variant="outline"
            className={`capitalize mt-1.5 ${statusBadgeClasses[status] ?? ''}`}
          >
            {status}
          </Badge>
        </div>
        ...
        <div className="space-y-3 text-sm">
          ...
          {center.whatsapp_number && (
            <div className="flex items-center gap-2 text-muted-foreground">
              ...
              <span>{center.whatsapp_number}</span>
            </div>
          )}
          <div className="flex items-center gap-2 text-muted-foreground">
            <Clock className="h-4 w-4 flex-shrink-0" />
            <span>
              {t('superAdmin.centerDetail.registeredOn')}{' '}
          ...
          {center.approved_at && (
            <div className="flex items-center gap-2 text-muted-foreground">
              ...
              <span>
                {t('superAdmin.centerDetail.approvedOn')}{' '}
                {format(new Date(center.approved_at), 'MMM d, yyyy')}
              </span>
            </div>
          )}
          {owner?.full_name && (
            <div className="flex items-center gap-2 text-muted-foreground">
              ...
              <span>
                {t('superAdmin.centerDetail.owner')}: {owner.full_name}
              </span>
            </div>
          )}
          {center.description && (
      ...
      <AlertDialog open={!!dialogAction} onOpenChange={() => setDialogAction(null)}>
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>
              {dialogAction ? dialogConfig[dialogAction].title : ''}
            </AlertDialogTitle>
            <AlertDialogDescription>
              {dialogAction ? dialogConfig[dialogAction].desc : ''}
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            ...
            <AlertDialogAction
              className={
                dialogAction && dialogConfig[dialogAction].destructive
