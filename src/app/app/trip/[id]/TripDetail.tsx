          ...
          <div className="container mx-auto px-6 sm:px-10 pb-8 md:pb-12">
            <div className="max-w-4xl animate-in fade-in slide-in-from-bottom-8 duration-1000">
              ...
              <p className="text-lg md:text-xl text-foreground font-light max-w-2xl flex items-center gap-2 bg-background/40 backdrop-blur-sm w-fit px-4 py-2 rounded-xl border border-foreground/10">
                ...
                <span className="opacity-80 text-base">{trip.departure_point}</span>
              </p>
            </div>
          </div>
        </div>
              ...
              <div>
                ...
                {canReview && myReview && (
                  <div className="bg-card p-6 rounded-3xl border border-white/5 shadow-card space-y-3">
                    ...
                    {myReview.body && (
                      <p className="text-sm text-muted-foreground whitespace-pre-line">{myReview.body}</p>
                    )}
                  </div>
                )}
                    ...
                    {tripReviews
                      ...
                      .map((r) => (
                        <div key={r.id} className="bg-card p-5 rounded-2xl border border-white/5 shadow-card">
                          <div className="flex items-center justify-between gap-3 mb-2">
                            <div>
                              <p className="font-semibold text-foreground">
                                {(r.diver_profiles?.full_name ?? '').trim().split(/\s+/)[0] ||
                                  t('reviews.verifiedAttendee')}
                              </p>
                          ...
                          {r.body && (
                            <p className="text-sm text-muted-foreground mt-1 whitespace-pre-line">{r.body}</p>
                          )}
                        </div>
                      ))}
                  </div>
            ...
            <div className="xl:sticky xl:top-28">
              <div className="bg-card rounded-3xl p-8 border border-white/5 shadow-2xl shadow-black/5 relative overflow-hidden">
                  ...
                  <div className="flex flex-col gap-3">
                    ...
                    {isCancellationRequested && (
                      <p className="text-sm text-warning text-center mt-2">
                        {t('diver.trip.cancellationPendingApproval')}
                      </p>
                    )}
                  </div>
                ...
                {!existingBooking && trip.available_spots > 0 && (
                  <p className="text-center text-xs text-muted-foreground mt-5 italic w-full">
