...
export default function DiverDashboard() {
  const { user } = useAuth();
  const { t } = useI18n();
    ...
    <div className="container mx-auto px-4 py-6 md:py-10 max-w-6xl space-y-10">
      <section className="relative rounded-3xl overflow-hidden bg-ocean-900 shadow-2xl shadow-primary/10">
        ...
        <div className="relative z-20 p-8 md:p-12 flex flex-col md:flex-row md:items-end justify-between gap-6">
          ...
          <Button asChild className="rounded-full bg-white text-primary hover:bg-white/90 font-bold px-8 shadow-lg">
            <Link href="/app/discover">{t('diver.dashboard.findExpeditions')}</Link>
          </Button>
        </div>
      </section>
          ...
          <Card className="bg-card border-border shadow-sm rounded-3xl overflow-hidden hover:border-primary/30 transition-colors">
            <CardContent className="p-6">
              ...
              <p className="text-xl font-black font-headline text-foreground">
                {CERT_LABELS[profile?.certification ?? ''] ?? t('profile.cert.none')}
              </p>
              <p className="text-[10px] text-muted-foreground uppercase tracking-widest font-bold mt-1">
                {t('diver.dashboard.certLevel')}
              </p>
            </CardContent>
          </Card>
        </div>
      ...
      <section>
        <div className="flex justify-between items-end mb-6">
          ...
          <Link href="/app/bookings" className="text-sm font-bold text-primary hover:underline flex items-center">
          ...
          {bookings.length === 0 ? (
            <div className="bg-background p-10 rounded-3xl text-center border border-dashed border-border flex flex-col items-center">
              ...
              <Button asChild variant="outline" className="rounded-full">
                <Link href="/app/discover">{t('diver.dashboard.browseTrips')}</Link>
              </Button>
            </div>
          ) : (
            bookings.map((booking) => (
              <Link key={booking.id} href={`/app/trip/${booking.trip_id}`} className="block group">
                  ...
                  <Badge
                    ...
                    className={
                      booking.status === 'confirmed'
                        ? 'bg-success/10 text-success border-success/20 px-3'
                        : 'px-3 capitalize'
                    }
                  >
                    {booking.status}
