...
const bookingStatusColors: Record<string, string> = {
  ...
  cancellation_requested:
    'bg-ocean-900/95 shadow-lg shadow-black/40 backdrop-blur-md text-warning border border-warning/30',
};
...
const DiverTripCard = memo(({ trip, linkTo, bookingStatus, eager = false }: Props) => {
  ...
  return (
    ...
    >
      ...
      <div className="absolute top-4 left-4 flex flex-col gap-2 z-10">
        {bookingStatus && (
          <span
            className={`text-[10px] font-bold uppercase tracking-widest px-3 py-1 rounded-full w-fit ${
              bookingStatusColors[bookingStatus] ||
              'bg-ocean-900/80 backdrop-blur-md border border-white/10 text-white'
            }`}
          >
        ...
        <div className="bg-ocean-900/85 backdrop-blur-lg rounded-xl p-4 sm:p-5 text-white border border-white/10 shadow-2xl">
          ...
          <div className="flex items-center gap-2 mb-4 flex-wrap">
            <Badge className="bg-white/10 hover:bg-white/20 text-white border-white/10 px-2 py-0.5 font-normal tracking-wide flex gap-1.5 items-center">
              ...
              <span className="text-xs text-ocean-200 truncate">
...
DiverTripCard.displayName = 'DiverTripCard';

export default DiverTripCard;
