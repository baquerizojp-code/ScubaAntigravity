...
import {
  AdminTripFormModal,
  type TripFormEditData,
} from '@/app/_components/AdminTripFormModal';
...
export default function AdminTrips() {
  ...
  }, [newParam, editParam, trips, router]);
  ...
  const filteredTrips = (() => {
    ...
    if (filterParam === 'upcoming') {
      ...
      return trips
        .filter((tr) => tr.status === 'published' && tr.trip_date >= today)
  ...
  return (
    <div>
        ...
        <div>
          ...
          <p className="text-sm text-muted-foreground">
            {filterParam === 'upcoming'
              ? t('admin.dashboard.upcomingTrips')
              : t('admin.trips.subtitle')}
          </p>
        </div>
        <div className="flex gap-2">
          {filterParam && (
            <Button variant="outline" onClick={() => router.replace('/admin/trips')}>
              {t('admin.nav.trips')}
            </Button>
          )}
          ...
          {filteredTrips.map((trip) => (
            ...
            >
              <div className="flex-1 min-w-0">
                ...
                <p className="text-sm text-muted-foreground">
                  {trip.dive_site} · {format(parseLocalDate(trip.trip_date), 'dd/MM/yyyy')} ·{' '}
                  {trip.trip_time?.slice(0, 5)}
                </p>
                <p className="text-sm text-muted-foreground">
                  ${Number(trip.price_usd)} · {trip.available_spots}/{trip.total_spots}{' '}
                  {t('common.spots')}
                </p>
              </div>
      ...
      <AdminTripFormModal open={dialogOpen} onOpenChange={setDialogOpen} trip={editingTrip} />
