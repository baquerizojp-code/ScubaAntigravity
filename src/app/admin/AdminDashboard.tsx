...
export default function AdminDashboard() {
  ...
  const upcomingTrips = useMemo(
    () => trips.filter((tr) => tr.status === 'published' && tr.trip_date >= today),
    [trips, today],
  );
  ...
  const totalRevenue = useMemo(
    () =>
      confirmedBookings.reduce(
        (sum: number, b: AdminBookingWithDetails) => sum + (Number(b.trips?.price_usd) || 0),
        0,
      ),
    [confirmedBookings],
  );
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        <Button onClick={() => router.push('/admin/trips?new=1')} className="gap-2">
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="bg-muted/40 text-[9px] sm:text-[10px] uppercase tracking-widest text-muted-foreground">
                <th className="px-2 py-3 sm:p-4 font-bold border-b border-border">
                  {t('admin.dashboard.colExpedition')}
                </th>
                <th className="px-2 py-3 sm:p-4 font-bold border-b border-border">
                  {t('admin.dashboard.colDate')}
                </th>
                <th className="px-2 py-3 sm:p-4 font-bold border-b border-border text-center">
                  {t('admin.dashboard.colCapacity')}
                </th>
                <th className="px-2 py-3 sm:p-4 font-bold border-b border-border text-right">
              ...
              {recentTrips.length === 0 ? (
                <tr>
                  <td colSpan={4} className="p-8 text-center text-muted-foreground">
                    {t('admin.dashboard.noExpeditions')}
                  </td>
                </tr>
              ) : (
                  ...
                  >
                    ...
                    <td className="px-2 py-3 sm:p-4">
                      <p className="text-xs sm:text-sm font-medium">
                        {format(new Date(trip.trip_date), 'dd/MM/yy')}
                      </p>
                    ...
                    <td className="px-2 py-3 sm:p-4">
                      <div className="flex flex-col items-center">
                        <span className="text-xs sm:text-sm font-bold">
                          {trip.total_spots - trip.available_spots}/{trip.total_spots}
                        </span>
                        <div className="w-full max-w-[40px] sm:max-w-[60px] h-1.5 bg-muted rounded-full overflow-hidden mt-1">
                          <div
                            className="h-full bg-primary rounded-full transition-all"
                            style={{
                              width: `${((trip.total_spots - trip.available_spots) / trip.total_spots) * 100}%`,
                            }}
                          ></div>
                        </div>
                      </div>
                    ...
                    <td className="px-2 py-3 sm:p-4 text-right">
                      <Badge
                        variant="outline"
                        className={`px-2 py-1 sm:px-3 sm:py-1 text-[9px] sm:text-[10px] uppercase tracking-widest border-0 ${
