...
| 10 | Next.js App Router migration | 3–4 weeks | ✅ PRs [#31](https://github.com/jpbaquerizo/ScubaTrip/pull/31)–[#38](https://github.com/jpbaquerizo/ScubaTrip/pull/38) |
...
### Session: April 17, 2026 — Item 10 completed (Phases A–G shipped)

**Phase A — Project setup** ([PR #31](https://github.com/jpbaquerizo/ScubaTrip/pull/31)). Installed `next`, `@supabase/ssr`, `@sentry/nextjs`. Scaffolded `app/` at the repo root alongside Vite. Added `next.config.ts` (Supabase `images.remotePatterns`), `tsconfig.json` extends (both bundlers typechecked), placeholder `app/page.tsx` rendering a "Next.js boot smoke-test" string. Added `dev:next`, `build:next`, `start:next` scripts; Vite `dev`/`build` untouched. `.env.local` created with `NEXT_PUBLIC_SUPABASE_*` duplicates of the existing `VITE_*` vars.

**Phase B — Auth plumbing** ([PR #32](https://github.com/jpbaquerizo/ScubaTrip/pull/32)). Dual Supabase clients: `src/integrations/supabase/browser.ts` (`createBrowserClient`) and `server.ts` (`createServerClient` reading cookies). Server helper `getSession()` in `src/app/_lib/auth.ts` wrapped in React `cache()` for per-request dedup. `AuthProvider` client component hydrates from server props and subscribes to `onAuthStateChange`. Remember-Me storage proxy preserved client-side.

**Phase C — Public pages as Server Components** ([PR #33](https://github.com/jpbaquerizo/ScubaTrip/pull/33)). `/`, `/explore`, `/explore/[slug]` ported to RSCs with `generateMetadata`, ISR `revalidate = 60`. HTML now ships pre-rendered — trip cards visible in view-source. `<TripCard>` split into a server presentational component and a client `<BookButton>` island. `next/image` used everywhere for srcset + AVIF/WebP.

**Phase D — Protected layouts** ([PR #34](https://github.com/jpbaquerizo/ScubaTrip/pull/34)). `src/app/app/layout.tsx`, `admin/layout.tsx`, `super-admin/layout.tsx` are server components that call `getSession()` and `redirect()` unauthorized users. `ProtectedRoute` component deleted. `diveCenterId` gate enforced server-side on the admin tree. Client layout shells (`DiverLayoutShell`, `AdminLayoutShell`, `SuperAdminLayoutShell`) receive session props from the server layout and hold the navbar/notifications.

**Phase E — Route migration** (PRs [#35](https://github.com/jpbaquerizo/ScubaTrip/pull/35), [#36](https://github.com/jpbaquerizo/ScubaTrip/pull/36), [#37](https://github.com/jpbaquerizo/ScubaTrip/pull/37)). Replaced all `react-router-dom` imports with `next/navigation` (`useRouter`, `usePathname`, `useSearchParams`) and `next/link`. Three PRs split the three role trees (diver, admin, super-admin) to keep each diff reviewable. Dynamic segments moved from `:id`/`:slug` to `[id]`/`[slug]` bracket folders.

**Phase F+G — Cutover** ([PR #38](https://github.com/jpbaquerizo/ScubaTrip/pull/38)). Removed `vite`, `@vitejs/plugin-react-swc`, `react-router-dom`, `@sentry/react`, `react-helmet-async`, `vitest-environment-happy-dom`, `@types/react-helmet`. Deleted `vite.config.ts`, `index.html`, `src/main.tsx`, `src/App.tsx`, `src/contexts/AuthContext.tsx`, `src/components/ProtectedRoute.tsx`, `src/components/{AdminLayout,DiverLayout,SuperAdminLayout,Navbar,NotificationBell,RoleSwitcher,TripCard,ThemeProvider,ErrorBoundary,PendingApprovalBanner}.tsx`, `src/hooks/useTripBooking.ts`, `src/test/test-utils.tsx`, `tsconfig.app.json`, `tsconfig.node.json`. Flipped scripts: `dev` → `next dev -p 3000`, `build` → `next build`, `start` → `next start`. Sentry swapped to `@sentry/nextjs` via `instrumentation.ts` + `instrumentation-client.ts` at repo root. `vercel.json` SPA rewrite removed; headers preserved. Playwright `baseURL` flipped to `http://localhost:3000` and `webServer` now spawns `npm run dev` (Next). CI workflow updated to require `NEXT_PUBLIC_SUPABASE_*` secrets (guard-skips when absent). Vercel framework preset flipped Vite → Next.js and `NEXT_PUBLIC_*` env vars added for Production + Preview + Dev. Production cutover verified: `/` and `/explore` ship fully-rendered HTML; all three roles (diver, dive center, super admin) smoke-tested on the preview deploy before merge. Post-merge test count: **105 tests across 8 files** (6 tests removed alongside deleted `AuthContext`/`ProtectedRoute`/`TripCard`/`ErrorBoundary`/`useTripBooking` files).
